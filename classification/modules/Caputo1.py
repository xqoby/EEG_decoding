...
class CaputoEncoder(nn.Module):
    def __init__(self, input_size=250, conv_channels=64, kernel_size=3, output_size=1024, alphas=[0.5, 1.0]):
        ...
        # 定义1D卷积层
        self.conv1d = nn.Conv1d(in_channels=input_size * len(alphas), 
                                out_channels=conv_channels, 
                                kernel_size=kernel_size, 
                                padding='same')

        # 定义输出层
        self.output = nn.Linear(conv_channels, output_size)
    ...
    def forward(self, x):
        ...
        # 转换为卷积层的输入形状
        x = x.permute(0, 2, 1)  # 调整维度为 (batch_size, channels, time_steps)

        # 将拼接后的特征传递给卷积层
        x = F.relu(self.conv1d(x))

        # 选取最后一个时间步的输出
        x = x.mean(dim=-1)  # 对时间维度取平均值
...
def test_model():
    ...
    # model = FreqEncoder(input_size=250, lstm_size=512, lstm_layers=2, output_size=1024)
    model = CaputoEncoder(input_size=250, conv_channels=64, kernel_size=3, output_size=1024)
    model = model.cuda()
